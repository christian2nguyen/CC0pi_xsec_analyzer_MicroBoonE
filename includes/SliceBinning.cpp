...
SliceBinning::SliceBinning( const std::string& config_file_name ) {
  ...
  // scheme
  std::cout<<"Inside: SliceBinning : Intizing a class "<< std::endl;
  ...
  std::cout<<"reading in slice_var "<< std::endl;
  SliceVariable temp_sv;
  for ( int v = 0; v < num_variables; ++v ) {
    in_file >> temp_sv;
    std::cout<< "temp_s = "<< temp_sv << std::endl;
  ...
  for ( int s = 0; s < num_slices; ++s ) {
    ...
    // variables.
    std::string slice_title = "" /*+ std::to_string( slices_.size() )*/;
...
std::ostream& operator<<( std::ostream& out, const Slice& slice )
{
  ...
  std::cout<<"final_axis_label = "<< final_axis_label << std::endl;
