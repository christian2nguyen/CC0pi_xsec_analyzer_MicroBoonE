    ...
    )
{
 ...
 THStack* Stack = (THStack*)Stack_input->Clone("Stack");
  ...
  double ymax = std::max( h_Total_MC->GetMaximum(), h_Total_MC->GetMaximum())* 1.7;
  if(YMax==99){YMax = ymax; }
 // slice_mc_plus_ext->hist_->GetMaximum() ) * 1.4;
  //if(DoBinWidt//hNorm==false) 
  h_Total_MC->SetMaximum(YMax);  
  h_Total_MC->GetYaxis()->SetRangeUser( 0., YMax );
  h_Total_MC->GetYaxis()->SetTitle( Yaxis_title );
  h_Total_MC->GetXaxis()->SetTitle( Xaxis_title );
  h_Total_MC->GetYaxis()->SetLabelSize(.03);
  h_Total_MC->GetXaxis()->SetLabelSize(.03);
  h_Total_MC->GetXaxis()->SetTitleSize(0.035);
  h_Total_MC->GetXaxis()->CenterTitle(kFALSE);
  h_Total_MC->SetTitleOffset (1.2,"Y");
  h_Total_MC->SetTitleOffset (.95,"X");
  h_Total_MC->SetLineWidth( 1 );
  h_Total_MC->Draw("hist " );
  Stack->Draw( "hist same" );
  //h_Total_MC->SetLineWidth( 0 );
  //h_Total_MC->Draw( "same hist e" );
  h_Total_MC->SetFillColorAlpha(Color, Errorshade);
...
void DrawFakeData(){
    ...
    textfake->DrawLatex(0.12, 0.86, "USING [NuWro] FAKE DATA");
