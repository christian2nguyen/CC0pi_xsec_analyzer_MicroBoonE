...
std::vector<std::string> ModelName_Global{"GENIE_v3_0_6","GENIEv2_12_10","NuWro_19_02_1","NEUT_5_4_0_1"}; 
...
void test_unfolding_ExtraModels() {
  ...
  std::string Pdf_name  = "CrossSection_2D_ExtraModels_WithOpenData";
 ...
 std::cout<<"Running : Test unfolding () "<< std::endl;
  ...
  const std::string respmat_file_name(
    "/exp/uboone/data/users/cnguyen/CC0Pi_Selection/EventSelection_5_13_2024/UnivMake_OPEN_Data_2D_binningscheme1_pmucorrection_v7_newbins.root" );
  //UnivMake_FakeData_BDTdecided_1D_v10_noBDTproton_pmucorrection.root
  ...
  // Do the systematics calculations in preparation for unfolding
  auto* syst_ptr = new MCC9SystematicsCalculator( respmat_file_name, "../systcalc_new.conf" ); //../ systcalc_unfold_fd.conf
 ...
 double total_pot = syst.total_bnb_data_pot_;
  ...
  for ( const auto& matrix_pair : matrix_map ) {
    ...
    unfolded_cov_matrix_map[ matrix_key ] = std::make_unique< TMatrixD >(
      ...
      if(DEBUG_PLOTS==true) visualizeMatrix(temp_mat, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf");
      
      sprintf(pdf_title, "err_prop::Matrix Key : %s  ", matrix_key.c_str());
          if(DEBUG_PLOTS==true) visualizeMatrix(err_prop, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf");
  ...
  visualizeMatrix(CombinedNorm, "Total Blockewise covariance matrix Norm + Mixed ",  "CrossSection_2D_ExtraModels_WithOpenData.pdf","Bin N", "Bin N");
  ...
  TMatrixD CorrelationMatrix =  covarianceToCorrelation(TOTAL_Cov);
   ...
   visualizeMatrix(TOTAL_Cov, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf","Bin N", "Bin N");
  ...
  visualizeMatrix(CorrelationMatrix, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf", "Bin N", "Bin N" , 1,-1);
  ...
  visualizeMatrix(Correlation_Shape_Matrix, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf", "Bin N", "Bin N" , 1,-1);
  ...
  visualizeMatrix(Correlation_Norm_Matrix, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf", "Bin N", "Bin N" , 1,-1);
  ...
  visualizeMatrix(Correlation_Mix_Matrix, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf", "Bin N", "Bin N" , 1,-1);
 ...
 //GridCanvas *Stack_FracError = new GridCanvas(uniq(), 3, 4, 800, 550);
   ...
   GC_Models_ERROR->SetRightMargin(.05);  
 ...
 if(DEBUG_PLOTS==true) visualizeMatrix(fake_data_truth, "fake_data_truth for CV",  "CrossSection_2D_ExtraModels_WithOpenData.pdf");
  // Save the GENIE CV model (before A_C multiplication) using a column vector
  // of event counts
  TMatrixD genie_cv_truth_vec( num_true_signal_bins, 1 );
  for ( int b = 0; b < num_true_signal_bins; ++b ) {
 ...
 if(DEBUG_PLOTS==true) visualizeMatrix(genie_cv_truth_vec, "genie_cv_truth_vec for CV",  "CrossSection_2D_ExtraModels_WithOpenData.pdf");
  // Multiply the truth-level GENIE prediction histogram by the additional
  // smearing matrix
  TMatrixD* A_C = result.add_smear_matrix_.get();
  multiply_1d_hist_by_matrix( A_C, genie_cv_truth );
  ...
  if ( using_fake_data ) {
    ...
    //visualizeMatrix(*fake_data_truth_hist, "fake_data_truth_hist",  "CrossSection_2D_ExtraModels_WithOpenData.pdf");
    multiply_1d_hist_by_matrix( A_C, fake_data_truth_hist );
 ...
 //////////////////////////////////////
  ...
  if ( USE_ADD_SMEAR ) {
    ...
    const TMatrixD& A_C = *result.add_smear_matrix_;
     ...
     visualizeMatrix(A_C, "Regularization Matrix : A_{C}",  "CrossSection_2D_ExtraModels_WithOpenData.pdf", "TRUE Bin N", "TRUE Regularized Bin N", 1.0, -1.0, BinN_lineMap); // 
    // Start with the fake data truth if present
    if ( using_fake_data ) {
      TMatrixD ac_truth( A_C, TMatrixD::kMult, fake_data_truth );
      fake_data_truth = ac_truth;
    ...
    for ( const auto& pair : generator_truth_map ) {
     ...
     std::cout<< "Tranformationg Model :: "<< pdf_title << std::endl;
      ...
      visualizeMatrix(*truth_mat, pdf_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf", "","",2);
...
// Looking the covarience matrix before slicing 
  ...
  /*
      for ( const auto& uc_pair : unfolded_cov_matrix_map ) {
        ...
        visualizeMatrix(*uc_matrix, matrix_title,  "CrossSection_2D_ExtraModels_WithOpenData.pdf", "","",2);
 ...
 for ( size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx ) {
    ...
    SliceHistogram* slice_cv = SliceHistogram::make_slice_histogram(
      genie_cv_truth_vec, slice, nullptr );
       ...
       if(DEBUG_PLOTS==true) DrawSlice(slice_cv,slice, "CrossSection_2D_ExtraModels_WithOpenData","Slice cv","bins",c1,60);
    ...
    if ( using_fake_data ) {
     ...
     if(DEBUG_PLOTS==true) DrawSlice(slice_truth,slice, "CrossSection_2D_ExtraModels_WithOpenData","Slice fake data before unfolding","bins",c1,60);
    ...
    slice_gen_map[ MicroBooNEType_string ] = slice_cv;
      ...
      if(DEBUG_PLOTS==true) DrawSlice(slice_cv,slice,"CrossSection_2D_ExtraModels_WithOpenData", "CV Slice","bins",c1,60);
      if(DEBUG_PLOTS==true) DrawSlice(slice_unf,slice,"CrossSection_2D_ExtraModels_WithOpenData", "unfolded Slice","bins",c1,60);
    ...
    for ( const auto& pair : generator_truth_map ) {
      ...
      //  *truth_mat, slice, nullptr );
        ...
        *truth_mat, slice, nullptr );
        ...
        if(DEBUG_PLOTS==true) DrawSlice(temp_slice,slice,"CrossSection_2D_ExtraModels_WithOpenData", modeltitle,"bins",c1,60);
        
        if(DEBUG_PLOTS==true) visualizeMatrix(*truth_mat, model_name,  "CrossSection_2D_ExtraModels_WithOpenData.pdf");
    ...
    for ( auto& pair : slice_gen_map ) {
      ...
      const auto& name = pair.first;
       ...
       std::string inputtitle = "Before Tranfermation , Model:" + name;
        ...
        if(DEBUG_PLOTS==true) DrawSlice(slice_h,slice,"CrossSection_2D_ExtraModels_WithOpenData", inputtitle,"bins",c1,60);
       ...
       if(DEBUG_PLOTS==true)  DrawSlice(slice_h,slice,"CrossSection_2D_ExtraModels_WithOpenData", inputtitleout,"bins",c1,60);
   ...
   */
   ...
   const std::vector< std::string > cov_mat_keys = { "total",
    ...
    //visualizeMatrix(*CovMatrix_, Name ,  "CrossSection_2D_ExtraModels_WithOpenData.pdf", " Bin N", "Bin N");
...
void test_unfolding_ExtraModels_Inclusive() {
 ...
 std::cout<<"Running : Test unfolding () "<< std::endl;
  ...
  const std::string respmat_file_name_inclusive(
    "/exp/uboone/data/users/cnguyen/CC0Pi_Selection/EventSelection_5_13_2024/UnivMake_Open_Data_2D_binningscheme2_pmucorrection_v6_newbins.root" );
  //UnivMake_FakeData_BDTdecided_1D_v10_noBDTproton_pmucorrection.root
  ...
  auto* sb_ptr = new SliceBinning( "../mybins_mcc9_2D_muon_inclusive_v1_july3_2024_noSideBands.txt");
  auto& sb = *sb_ptr;
  // Do the systematics calculations in preparation for unfolding
...
int main() {
  ...
  BinN_lineMap_scheme2.insert(std::pair<std::string, double>("Slice9", 44 ));
    ...
    std::string NuwrotupleName = input_directory + "UnivMake_FakeData_2D_binningscheme2_pmucorrection_v3_Closure.root";
     ...
     //unfolding_NuWroClosure_tests(NuwrotupleName,  "../mybins_mcc9_2D_muon_inclusive_v1_july3_2024_noSideBands.txt","BinScheme2_NuWro_v3" ,true, 16, BinN_lineMap_scheme2);
    ...
    //unfolding_NuWroClosure_tests(NuwrotupleName2,  "../mybins_mcc9_2D_muon_v1_july3_2024_noSideBands.txt","BinScheme1_NuWro_v4", true, 16,BinN_lineMap);
  
  
    //unfoldingGENIEClosure("/exp/uboone/data/users/cnguyen/CC0Pi_Selection/EventSelection_5_13_2024/UnivMake_FakeData_2D_binningscheme2_pmucorrection_v3_Closure.root",
    // "../mybins_mcc9_2D_muon_inclusive_v1_july3_2024_noSideBands.txt", "binning_scheme2_new2",BinNSlice, BinN_lineMap_scheme2);
...
char TitleInput[1024];
 ...
 lg_input->Draw("same");
  GridCanvas_input->SetYLabel_Size(.01);
...
bool removebins, int Removesideband_number, std::map<std::string, double> lines) {
 ...
 std::cout<<"Running : Test unfolding () "<< std::endl;
  ...
  // Do the systematics calculations in preparation for unfolding
  auto* syst_ptr = new MCC9SystematicsCalculator( respmat_file_name, "../systcalc_NuWroClosure_v2.conf" );
  //auto* syst_ptr = new MCC9SystematicsCalculator( respmat_file_name, "../systcalc.conf" );
  auto& syst = *syst_ptr;
