...
constexpr double DEFAULT_MATRIX_INVERSION_TOLERANCE = 1e-4; // Matrix is not inverting for 2D inclusing changing TOLERANCEe 1e-4 to 1e-3 
  ...
  const double inversion_tolerance = DEFAULT_MATRIX_INVERSION_TOLERANCE )
{
  ...
  for ( int a = 0; a < num_bins; ++a ) {
    for ( int b = 0; b < num_bins; ++b ) {
      ...
      std::cout<< "abs_diff = "<< abs_diff << " inversion_tolerance = "<< inversion_tolerance <<std::endl;
...
void divideMatrixByConstant(TMatrixD& matrix, Double_t constant) {
    Int_t numRows = matrix.GetNrows();
    Int_t numCols = matrix.GetNcols();

    // Loop through each element of the matrix and divide by the constant
...
void MultiplyMatrixByConstant(TMatrixD& matrix, Double_t constant) {
    Int_t numRows = matrix.GetNrows();
    Int_t numCols = matrix.GetNcols();
