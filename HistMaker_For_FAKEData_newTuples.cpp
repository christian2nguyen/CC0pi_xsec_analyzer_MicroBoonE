...
///
 ...
 if(input_Int ==0 ){ Rootfile1name = "ru-stv_nuwro_fakedata_peleeTuple_uboone_run1.root";}
 else if(input_Int ==1 ){ Rootfile1name = "ru-stv_nuwro_fakedata_peleeTuple_uboone_run2.root";}
  else if(input_Int ==2 ){ Rootfile1name = "ru-stv_nuwro_fakedata_peleeTuple_uboone_run3.root";}
   else if(input_Int ==3 ){ Rootfile1name = "rustv-data_extbnb_mcc9.1_v08_00_00_25_reco2_D_E_all_reco2.root";}
   else if(input_Int ==4 ){ Rootfile1name = "ru-stv_high_stat_prodgenie_bnb_nu_overlay_DetVar_Run1_NuWro_reco2_reco2.root";}
...
h_trk_distance_v->SetDirectory(0);h_trk_distance_v->Sumw2();
  ...
  bool mc_signal, sel_signal,sel_signal_CCNP0Pi, sel_muon_contained;
  ...
  int sel_num_proton_candidates,sel_num_pion_candidates; 
...
bool sel_has_pion_candidate , sel_has_pi_candidate ;


//int sel_num_pion_candidates; 
...
stv->SetBranchAddress("sel_has_pion_candidate",&sel_has_pion_candidate);
...
//sel_has_pion_candidate && sel_has_pi_candidate && sel_num_pion_candidates > 0
...
CCZeroPi_type TopologyType = EventCategory_tool.returnCCZeroPi_type_ForData(sel_num_proton_candidates);
...
std::vector<int> BDTPID_Prediction_Muon_front = MakereorderVector(*xgb_pid_vec, BDT_Category::kBDT_Muon);
...
//h_CCZeroP_ProtonCandidate_BTDPrediction->Fill(score_3,Wgt);
 ...
 else if (sel_num_proton_candidates ==1){
 ...
 else if (sel_num_proton_candidates ==2)
 {
  if(index1==1){
...
//for(auto vec:BinMap.second ){std::cout<<vec<<",";}
  ...
  TH1D* Bin =  h_costheta_Pmu_UBTH2Poly_inclusive->Projection(HistName, BinMap.second); 
  Bin->Write(HistName);
}
