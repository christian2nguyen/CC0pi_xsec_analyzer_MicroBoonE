...
void make_config_mcc9_2D_muon_CC0Pi() {
  ...
  std::string selection = "sel_CC0pi && sel_BDT_NotBOGUS  && sel_All_Protons_BDT_SCORE_above_threshold";
  ...
  // make a LaTeX table
  std::ofstream tex_bin_table_file( "mybintable_mcc9_muon2D_CC0pi.tex" );
...
void make_config_mcc9_2D_muon_CC0Pi_inclusive() {
  ...
  // Also save the bin definitions to an output file that will be used to
  // make a LaTeX table
  std::ofstream tex_bin_table_file( "mybintable_mcc9_muon2D_CC0pi_inclusive.tex" );
  tex_bin_table_file << "\\documentclass{standalone}\n"
    << "\\usepackage{booktabs}\n"
    << "\\usepackage{siunitx}\n"
    << "\\DeclareSIUnit\\clight{\\text{\\ensuremath{c}}}\n"
