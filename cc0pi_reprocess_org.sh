...
if [ "$#" -ne "$num_expected" ]; then
  echo "Usage: ./cc0pi_reprocess.sh OUTPUT_DIRECTORY NTUPLE_LIST_FILE"
  exit 1
fi
...
while read line; do
  ...
  if [[ ! $line = \#* ]] && [[ $line = *[^[:space:]]* ]]; then
    ...
    output_file_name="${output_dir}/rustv-$(basename ${input_file_name})"
    echo "PROCESSING ${input_file_name} --> ${output_file_name}"
    time cc0pi_analyzer_org ${input_file_name} ${output_file_name}
  fi
